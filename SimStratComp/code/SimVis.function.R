...
get.oc.aggr<-function(...){
  ...
  riskcat<-sim.data$Y000[,rc.name]%>%unique%>%sort
  outcome<-MDL.setup$Colname[MDL.setup$VarCat=="Outcome"]
  oc.aggr.temp<-tibble()
  for(rc in riskcat){
    rind<-sim.data$Y000[,rc.name]==rc
    for(oc in outcome){
      temp<-tibble(
        RiskCat=rc,
        Outcome=oc,
        Mean=sim.data$Y000[rind,oc]%>%mean(na.rm=T),
        SD=sim.data$Y000[rind,oc]%>%sd(na.rm=T)
      )
...
get.sim<-function(fd){
  ...
  for(y in 0:n.year){
    fn<-fn.simp[[y+1]]
    sim.data[[fn]]<-readRDS(str_c(fd,fn.list[y+1]))
...
#sim.fd<-"risk engine/RUN/SIM/SIM20180404005711/"
#sim.fd<-"risk engine/RUN/SIM/SIM20180404090836/"
...
get.ind.list<-function(...){
  ...
  IV<-MDL.setup$Colname[MDL.setup$VarCat=="Intervention"]
  if(length(IV)>0){
    for(iv in IV){
...
get.prev<-function(ind){
  ...
  for(var in bin.var){
    prev<-sapply(sim.data.sub,function(x){
      if(var=="Death"){
        rind<-1:nrow(x)
      }else{
        rind<-x$Death==0
      }
      x[rind,var]%>%mean(na.rm=T)
    })
...
get.cat.prog<-function(ind){
  ...
  cat.prop.list<-list()
  for(var in fac.var){
    df<-sapply(sim.data.sub,function(x){
      if(var=="RiskCat"){
        rind<-1:nrow(x)
      }else{
        rind<-x$Death==0
...
get.OC.by.yr<-function(ind){
  ...
  OC.by.yr<-cbind(
    year=0:nyear,
    # remove mortality cat
    as.matrix(RiskCat[,-c(1,10)])%*%as.matrix(oc.aggr$mean[,-1])
  )%>%as.tibble
